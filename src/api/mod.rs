...
pub fn create_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/metrics/summary",
            get(metrics_handler::metrics_summary),
        )
        ...
        .route(
            "/api/test-plans",
            get(test_plans::list_test_plans).post(test_plans::create_test_plan),
        )
        .route(
            "/api/test-plans/{id}/collection",
            put(test_plans::move_test_plan),
        )
        .route(
            "/api/test-plans/{id}",
            get(test_plans::get_test_plan)
                .put(test_plans::update_test_plan)
                .delete(test_plans::delete_test_plan),
        )
        // ── Collections ───────────────────────────────────────────────────────
        .route(
            "/api/collections",
        ...
        .route(
            ...
            get(executions::list_executions)
                .post(executions::enqueue_execution)
                .delete(executions::clear_executions),
        )
        .route(
            "/api/executions/{id}",
        ...
        .route(
            "/api/specs",
            get(specs::list_specs).post(specs::upload_spec),
        )
